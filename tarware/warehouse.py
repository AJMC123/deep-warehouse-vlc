...
# Tentativa de importar pyastar2d (com fallback para Python puro)
try:
    import pyastar2d
except ImportError:
    pyastar2d = None
    print("[Aviso] pyastar2d não encontrado — a simulação vai usar pathfinding em Python (mais lento).")

from tarware.definitions import Action, AgentType, Direction, RewardType, CollisionLayers
...
class Warehouse(gym.Env):
    ...
    def find_path(self, start, goal: Tuple[int], agent, care_for_agents: bool = True) -> List[Tuple[int]]:
        """
        Mantém a lógica original:
        - start e goal chegam como (y, x)
        - grid indexa [y, x]
        - devolve caminho em (x, y)
        Usa pyastar2d se existir; senão, A* puro em Python.
        """
        import numpy as np
        import heapq
        from tarware.definitions import AgentType, CollisionLayers

        # -------- 1) Grid de custos/obstáculos --------
        ...
        if agent.type == AgentType.PICKER:
            # Pickers: só highways, mas podem aceder ao goal
            grid += (1 - self.highways)
            grid[goal[0], goal[1]] -= (not self._is_highway(goal[1], goal[0]))
            # bloquear última linha (comportamento original)
            for i in range(self.grid_size[1]):
                grid[self.grid_size[0] - 1, i] = 1

        # Evitar “atalho” lateral por racks para Pickers
        start_fix = (0, 0)  # em (dy, dx)
        if (
            agent.type == AgentType.PICKER
            and (not self._is_highway(start[1], start[0]))  # _is_highway(x,y)
            and goal[0] == start[0]
            and abs(goal[1] - start[1]) == 1
        ):
            if self._is_highway(start[1] - 1, start[0]):
                start_fix = ( -1, 0)
            if self._is_highway(start[1] + 1, start[0]):
                start_fix = (  1, 0)
            grid[start[0], start[1]] = 1  # força contorno

        # liberta a célula de arranque (y+dy, x+dx)
        sy, sx = start[0] + start_fix[0], start[1] + start_fix[1]
        grid[sy, sx] = 0
        ...
        start_fixed = (sy, sx)  # (y, x)

        # -------- 2) pyastar2d se existir --------
        if pyastar2d is not None:
            astar_path = pyastar2d.astar_path(
                grid,
                start_fixed,   # (y, x)
                goal,          # (y, x)
            ...
            if astar_path is not None:
                astar_path = [tuple(x) for x in list(astar_path)]  # [(y,x), ...]
                # mesma regra do original
                astar_path = astar_path[1 - int(grid[start[0], start[1]] > 1):]
            # devolver em (x, y)
            return [(x, y) for (y, x) in astar_path] if astar_path else []

        # -------- 3) Fallback A* em Python (4-vizinhos) --------
        H, W = grid.shape
        ...
        def neighbors(y, x):
            for dy, dx in ((-1,0), (1,0), (0,-1), (0,1)):
                ny, nx = y + dy, x + dx
                if in_bounds(ny, nx) and passable(ny, nx):
                    yield ny, nx

        def heuristic(a, b):
            return abs(a[0] - b[0]) + abs(a[1] - b[1])  # Manhattan

        start_node = (start_fixed[0], start_fixed[1])  # (y, x)
        goal_node  = (goal[0], goal[1])                # (y, x)

        if not passable(*start_node) or not passable(*goal_node):
            return []
        ...
        while open_heap:
            ...
            if current == goal_node:
                ...
                while node is not None:
                    ...
                    node = came_from[node]
                path.reverse()
                # remover o start se redundante (mesma regra)
                if len(path) > 0 and path[0] == start_fixed:
                    path = path[1:]
                # devolver em (x, y)
            ...
            for ny, nx in neighbors(cy, cx):
                tentative = g_score[current] + float(grid[ny, nx])
                if tentative >= 1e15:
                    continue
                if (ny, nx) not in g_score or tentative < g_score[(ny, nx)]:
                    g_score[(ny, nx)] = tentative
                    f = tentative + heuristic((ny, nx), goal_node)
